	...
	<body>
		<table width="700" border="0" cellspacing="0" cellpadding="0" align="center" >
 	    ...
 	    <tr>
			...
			{{ trans('words.mail_response_pet_1') }} ({{ $pet_name }}). {{ trans('words.mail_response_pet_2') }}
			<br><br>
			@if($your_name != '')
			{{ trans('words.name') }}: {{ $your_name }}<br>
			@endif
			@if($your_phone != '')
			{{ trans('words.phone') }}: {{ $your_phone }}<br>
			@endif
			@if($your_email != '')
			{{ trans('words.email') }}: {{ $your_email }}<br>
			@endif
			@if(isset($location) && $location != '')
			{{ trans('words.location') }}: {{ $location }}<br>
			@endif
			{{ trans('words.message') }}: {{ $your_message }}
			<br><br>
